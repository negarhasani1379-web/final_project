...
class SalaryEndToEndTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.teacher_a = User.objects.create_user(
            username="e2e_teacher_a",
            password="Test12345",
            role=UserRole.TEACHER,
        )

        self.teacher_b = User.objects.create_user(
            username="e2e_teacher_b",
            password="Test12345",
            role=UserRole.TEACHER,
        )

        self.teacher_c = User.objects.create_user(
            username="e2e_teacher_c",
            password="Test12345",
            role=UserRole.TEACHER,
        )
    ...
    def create_report(
        ...
        response = self.client.post(
            ...
            {
                "session": session_id,
                "teacher_assignment": assignment_id,
    ...
    def calculate_salary(
        ...
        return self.client.post(
            "/api/teacher-monthly-salary/calculate/",
            {
                "teacher": teacher_id,
                "year": year,
                "month": month,
    ...
    def test_full_salary_flow(self):
        school_id = self.create_school()

        term_id = self.create_term()

        class_60_id = self.create_class(
            school_id=school_id,
            term_id=term_id,
            title="E2E Class 60",
            session_duration=60,
        ...
        class_120_id = self.create_class(
            school_id=school_id,
            term_id=term_id,
            title="E2E Class 120",
            session_duration=120,
        ...
        assignment_60_id = self.create_assignment(
            teacher_id=self.teacher_a.id,
            classroom_id=class_60_id,
        )

        assignment_90_id = self.create_assignment(
            teacher_id=self.teacher_a.id,
            classroom_id=class_90_id,
        )

        assignment_120_id = self.create_assignment(
            teacher_id=self.teacher_a.id,
            classroom_id=class_120_id,
        )

        now = timezone.now()

        session_60_id = self.create_session(
            classroom_id=class_60_id,
            session_number=1,
            session_date=now - timedelta(hours=1),
        )

        session_90_id = self.create_session(
            classroom_id=class_90_id,
            session_number=1,
            session_date=now - timedelta(hours=2),
        )

        session_120_id = self.create_session(
            classroom_id=class_120_id,
            session_number=1,
            session_date=now - timedelta(hours=3),
        )

        report_60_id, report_60_data = self.create_report(
            teacher=self.teacher_a,
            session_id=session_60_id,
            assignment_id=assignment_60_id,
            lesson_summary="60 minute lesson",
        )

        report_90_id, report_90_data = self.create_report(
            teacher=self.teacher_a,
            session_id=session_90_id,
            assignment_id=assignment_90_id,
            lesson_summary="90 minute lesson",
        )

        report_120_id, report_120_data = self.create_report(
            teacher=self.teacher_a,
            session_id=session_120_id,
            assignment_id=assignment_120_id,
            lesson_summary="120 minute lesson",
        )

        self.assertFalse(
            report_60_data["is_late"]
        )

        self.assertFalse(
            report_90_data["is_late"]
        )

        self.assertFalse(
            report_120_data["is_late"]
        )

        self.review_report(
            report_id=report_60_id,
            new_status=SessionReportStatus.APPROVED,
        )

        self.review_report(
            report_id=report_90_id,
            new_status=SessionReportStatus.APPROVED,
        )

        self.review_report(
            report_id=report_120_id,
            new_status=SessionReportStatus.APPROVED,
        )
        ...
        base_rate = Decimal("100000.00")

        rate_data = self.create_teacher_rate(
            teacher_id=self.teacher_a.id,
            term_id=term_id,
            base_rate=base_rate,
        )
        ...
        self.assertEqual(
            Decimal(rate_data["base_rate"]),
            base_rate,
        )

        response = self.calculate_salary(
            teacher_id=self.teacher_a.id,
            year=now.year,
            month=now.month,
        )
    ...
    def test_salary_cannot_be_calculated_with_pending_report(self):
        school_id = self.create_school()
        ...
        session_1_id = self.create_session(
            classroom_id=class_id,
            session_number=1,
            session_date=now - timedelta(hours=1),
        )

        session_2_id = self.create_session(
            classroom_id=class_id,
            session_number=2,
            session_date=now - timedelta(hours=2),
        )

        report_1_id, _ = self.create_report(
    ...
    def test_salary_cannot_be_calculated_with_rejected_report(self):
        school_id = self.create_school()
        ...
        class_id = self.create_class(
            school_id=school_id,
            term_id=term_id,
            title="Rejected Test Class",
            session_duration=90,
        )

        assignment_id = self.create_assignment(
            teacher_id=self.teacher_c.id,
            classroom_id=class_id,
        )
        ...
        report_1_id, _ = self.create_report(
            teacher=self.teacher_c,
            session_id=session_1_id,
            assignment_id=assignment_id,
            lesson_summary="Approved report",
        )

        report_2_id, _ = self.create_report(
            teacher=self.teacher_c,
            session_id=session_2_id,
            assignment_id=assignment_id,
            lesson_summary="Rejected report",
        ...
        self.review_report(
            ...
            new_status=SessionReportStatus.REJECTED,
            comment="Attendance needs correction.",
        )
